...
When the user asks you to lint or audit the wiki, use the `/lint` skill,
which:

- Checks for contradictions between pages
- Finds orphan pages (no inbound links from other pages)
- Identifies concepts mentioned in pages that lack their own page
- Flags claims that may be outdated based on newer sources
- Checks that all pages follow the page format above
- Reports findings as a numbered list with suggested fixes
